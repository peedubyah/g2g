...
# Function to calculate the trends
def calculate_trends(data, current_time):
    trends = {}
    for period in [12, 24, 72]:  # 12 hours, 24 hours, 3 days
        ...
        if not past_data.empty:
            trends[f"{period}H"] = {
                ...
                "duriel_ticket_price": data.iloc[-1]['duriel_ticket_price'] - past_data.iloc[-1]['duriel_ticket_price'],
                "varshan_ticket_price": data.iloc[-1]['varshan_ticket_price'] - past_data.iloc[-1]['varshan_ticket_price'],
            }
    return trends
...
def post_to_discord(gold_value, duriel_ticket_price, varshan_ticket_price, trends):
    ...
    for period, trend in trends.items():
        trend_lines.append(
            f"**{period} Trend:**\n"
            f"1 Million Gold: ${trend['gold_value']:+.2f}\n"
            f"Duriel Ticket: ${trend['duriel_ticket_price']:+.2f}\n"
            f"Varshan Ticket: ${trend['varshan_ticket_price']:+.2f}\n"
        )
    trend_description = "\n".join(trend_lines)
...
def main():
    try:
        ...
        if gold_value and duriel_ticket_price and varshan_ticket_price:
            ...
            # Calculate trends
            trends = calculate_trends(data, datetime.now())
            
            post_to_discord(gold_value, duriel_ticket_price, varshan_ticket_price, trends)
        else:
