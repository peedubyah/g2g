...
def scrape_gold_value():
    ...
    data = response.json()
    converted_unit_price = data['payload']['results'][0]['converted_unit_price'] * 1000
    formatted_price = f"{converted_unit_price:.2f}"
...
# Function to scrape the Andariel Ticket price
@retry(**retry_settings)
def scrape_andariel_ticket_price():
    url = 'https://sls.g2g.com/offer/search?service_id=0765978e-3fdf-48b4-bed3-184823aa439e&brand_id=lgc_game_26891&filter_attr=33821c26:0a926d8a%7C59dd7f4c:9466f8f6%7C9870fe77:328ce087&page_size=20&group=0&currency=USD&country=US'
    headers = {
        'accept': 'application/json, text/plain, /',
        'accept-language': 'en-US,en;q=0.9',
        'dnt': '1',
    ...
    data = response.json()
    formatted_unit_price = data['payload']['results'][0]['formatted_unit_price']
    logging.info(f"Scraped Andariel Ticket price: {formatted_unit_price}")
    return formatted_unit_price

# Function to scrape the Stygian Stone price
@retry(**retry_settings)
def scrape_stygian_stone_price():
    url = 'https://sls.g2g.com/offer/search?service_id=0765978e-3fdf-48b4-bed3-184823aa439e&brand_id=lgc_game_26891&filter_attr=33821c26:0a926d8a%7C59dd7f4c:9466f8f6%7C9870fe77:328ce087&page_size=20&group=0&currency=USD&country=US'
    headers = {
        'accept': 'application/json, text/plain, /',
        'accept-language': 'en-US,en;q=0.9',
    ...
    data = response.json()
    formatted_unit_price = data['payload']['results'][0]['formatted_unit_price']
    logging.info(f"Scraped Stygian Stone price: {formatted_unit_price}")
    return formatted_unit_price

# Function to send the values to Discord webhook
def post_to_discord(gold_value, duriel_ticket_price, varshan_ticket_price, andariel_ticket_price, stygian_stone_price):
    webhook_url = 'https://discord.com/api/webhooks/1249227360964972688/51cP4-HebSc3HkZ_2Vex9Dq2zBodTVJGOkTItVO9VzGtM0xMM4t7Kok-gQ4e37COdxpW'
    timestamp = datetime.now().isoformat()
    embed = {
        "title": "G2G Update",
        "description": (
            f":coin: [**1 Billion Gold**](https://www.g2g.com/categories/diablo-4-gold): **${gold_value}**\n"
            f":egg: [**Duriel Ticket Price**](https://www.g2g.com/categories/diablo-4-item-buy/offer/group?fa=33821c26%3A0a926d8a%7C59dd7f4c%3Af6477539%7C9870fe77%3A328ce087): **${duriel_ticket_price}**\n"
            f":bone: [**Varshan Ticket Price**](https://www.g2g.com/categories/diablo-4-item-buy/offer/group?fa=33821c26%3A0a926d8a%7C59dd7f4c%3A4cce7c55%7C9870fe77%3A328ce087): **${varshan_ticket_price}**\n"
            f":chains: [**Andariel Ticket Price**](https://www.g2g.com/categories/diablo-4-item-buy/offer/group?fa=33821c26%3A0a926d8a%7C59dd7f4c%3A9466f8f6%7C9870fe77%3A328ce087): **${andariel_ticket_price}**\n"
            f":rock: [**Stygian Stone Price**](https://www.g2g.com/categories/diablo-4-item-buy/offer/group?fa=33821c26%3A0a926d8a%7C59dd7f4c%3A9466f8f6%7C9870fe77%3A328ce087): **${stygian_stone_price}**"
        ),
...
def main():
    try:
        ...
        andariel_ticket_price = scrape_andariel_ticket_price()
        stygian_stone_price = scrape_stygian_stone_price()
        if gold_value and duriel_ticket_price and varshan_ticket_price and andariel_ticket_price and stygian_stone_price:
            post_to_discord(gold_value, duriel_ticket_price, varshan_ticket_price, andariel_ticket_price, stygian_stone_price)
        else:
